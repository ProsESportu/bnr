static WEATHER_STATIONS: &[(&str, f64)] = &[
    ("Abha", 18.0),
    ("Abidjan", 26.0),
    ("Abéché", 29.4),
    ("Accra", 26.4),
...
fn main() {
    ...
    for (unit, zeros) in SIZE_MAP {
        if size.ends_with(unit) {
            size = size.replace(unit, zeros);
        }
    }
    ...
    for _i in 0..thread_count {
        handles.push(std::thread::spawn(move || {
            ...
            for _ in 0..(size / thread_count as u64) {
                let (city, mean) = WEATHER_STATIONS.choose(&mut rng).unwrap();
                let temp: f64 = rand_distr::Normal::new(*mean, 7.0)
                    .unwrap()
                    .sample(&mut rng);
                writeln!(&mut writer, "{};{:.1}", city, temp).unwrap();
            }
            vec
        }));
    }
